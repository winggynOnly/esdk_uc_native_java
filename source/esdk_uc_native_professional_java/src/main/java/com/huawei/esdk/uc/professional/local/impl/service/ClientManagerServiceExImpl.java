...
public class ClientManagerServiceExImpl implements ClientManagerServiceEx {
	...
	public Integer userLogin(String ucAccount, String password,
        ...
        try
        {
            pwd = Base64Utils.encode(AES128Utils.encode(password.getBytes("UTF-8")));
        }
        catch (Exception e1)
        {
            return PlatformNativeConstant.SDK_UC_PASSWORD_ENCODE_ERRORCODE;
        }
		try {
			return ucProfessionalClientCommon.userLogin(ucAccount, pwd,
					registerFunc);
		} catch (Exception e) {
